...
class InProcessResult:
    ...
    def compare_versus_expected(
        ...
        if additional_text:
            ...
            LOGGER.warning(
                "actual>>%s",
                InProcessResult.make_value_visible(actual_stream.getvalue()),
            )
            print(
                f"WARN>actual>>{InProcessResult.make_value_visible(actual_stream.getvalue())}"
            )
            LOGGER.warning(
                "expect>>%s", InProcessResult.make_value_visible(expected_text)
            )
            print(f"WARN>expect>>{InProcessResult.make_value_visible(expected_text)}")
            if log_extra:
